...
\begin{frame}[t]
  ...
  \begin{center}
    \begin{tikzpicture}
      \node[draw=bgstuBlue, line width=1pt, minimum width=2.4cm, minimum height=1.0cm,
            rounded corners=3pt, fill=bgstuLight] at (0,0)
            {\color{bgstuBlue}\small\textbf{ИИТУС}};
      \node[draw=bgstuBlue, line width=1pt, minimum width=2.4cm, minimum height=1.0cm,
            rounded corners=3pt, fill=bgstuLight] at (3.8,0)
            {\color{bgstuBlue}\small\textbf{БГТУ}};
    \end{tikzpicture}
  \end{center}
...
\begin{frame}{Постановка задачи на разработку ПО}
  ...
  \vspace{0.4em}
  \begin{block}{Нефункциональные требования (из ТЗ)}
    \fontsize{8}{9}\selectfont
    GPU $\geq$16\,ГБ VRAM~~$\cdot$~~
    RAM $\geq$32\,ГБ~~$\cdot$~~
    Диск $\geq$1\,ТБ~~$\cdot$~~
    Канал $\geq$10\,ГБит/с~~$\cdot$~~
    Восстановление $\leq$1\,ч
  \end{block}
\end{frame}
...
\begin{frame}{Используемые технологии, инструменты и библиотеки}
  \begin{columns}[t]
    ...
    \column{0.34\textwidth}
      \textbf{Протоколы и стандарты}
      \begin{itemize}
        \item REST API
        \item WebSocket + STOMP
        \item HTTP Chunked Transfer
        \item JWT / HS256
        \item FLAC + нейросетевой кодек
      \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Описание обрабатываемых данных, форматы и структура}
  \begin{columns}[t]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        \item \texttt{users} --- хэш пароля + соль
        \item \texttt{author}, \texttt{group}, \texttt{author\_group}
        \item \texttt{song} --- идентификатор (неизменяем)
        \item \texttt{song\_release} --- версия: тег, BPM, тональность, слова, файлы
        \item \texttt{file} --- MIME, имя, длительность; тело на диске
        \item \texttt{playlist}, \texttt{playlist\_song} --- фильтр по тегу/версии
        \item \texttt{comment}, \texttt{comment\_song} --- чат
      \end{itemize}
    \column{0.5\textwidth}
      \textbf{Аудиоданные:}
      \begin{itemize}
...
% ---- СЛАЙД 8: Входные/выходные параметры ----
\begin{frame}{Описание входных и выходных параметров системы}
  \begin{columns}[t]
    \column{0.5\textwidth}
      \textbf{Входные параметры:}
      \begin{itemize}
        \item \textbf{Аутентификация:} email, пароль
        \item \textbf{Сущности:} имя, описание, аватар, связи
        \item \textbf{Релиз:} тег, BPM, тональность, слова, файлы, флаг ведущего файла
        \item \textbf{Плейлист:} имя, описание, \{song\_id, фильтр\}
        \item \textbf{Файл:} multipart, флаг сжатия кодеком
        \item \textbf{Плеер:} id версии / плейлиста
      \end{itemize}
    \column{0.5\textwidth}
      \textbf{Выходные параметры:}
      \begin{itemize}
        \item \textbf{API:} JSON (Pydantic-схемы)
        \item \textbf{Файл:} скачивание по ссылке
        \item \textbf{Аудио (кодек):} HTTP Chunked, потоковые фреймы
        \item \textbf{JWT:} Access Token (HS256), содержит id, username, email
        \item \textbf{Healthcheck:} \texttt{200 OK}
        \item \textbf{Документация:} Swagger / OpenAPI (авто)
      \end{itemize}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[t]
    \column{0.5\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{1pt}
        \item Nginx → Backend / Frontend
        \item Backend → PostgreSQL + FS
        \item NVIDIA Runtime для PyTorch
      \end{itemize}
    \column{0.5\textwidth}
      ...
      \end{center}
      Repository → Service → Router\\[0.2em]
      \textbf{Фронтенд: Feature Sliced Design}\\
      {\small\texttt{Shared → Entity → Features → Widgets → Pages → App}}
  \end{columns}
...
\begin{frame}{Внешний вид программы}
  \begin{columns}[t]
    \column{0.5\textwidth}
      \textbf{Формы:}
      \begin{center}
        \includegraphics[width=60mm]{figures/pic3_12.png}\\[0.4em]
      \end{center}
    \column{0.5\textwidth}
      \textbf{Основные экраны:}
      \begin{center}
        \includegraphics[width=60mm]{figures/pic3_11.png}\\[0.4em]
      \end{center}
  \end{columns}

  \vspace{0.4em}
  \textbf{Ключевые UI-компоненты:}
  \begin{itemize}
    \item Карточки авторов/групп с формами редактирования
    \item Страница композиции: история версий, чат, файлы, плейлисты
    \item Плейлист с умным фильтром (тег / id / последняя версия)
    \item Плеер: shuffle / loop / очередь / громкость / перемотка
  \end{itemize}
\end{frame}
...
\begin{frame}{Заключение}
  ...
  \begin{itemize}
    \item \textbf{Исследованы} алгоритмы хранения аудио (FLAC, MP3, AAC, Opus); выбрана гибридная схема FLAC + автоэнкодер
    ...
    \item \textbf{Реализован} собственный формат с потоковой распаковкой
    \item \textbf{Достигнуто} SNR~=~17\,дБ при сжатии $\times 2$
    \item \textbf{Проведено} Smoke-тестирование всех функциональных требований
    \item \textbf{Составлено} руководство пользователя
  \end{itemize}

  \vspace{0.3em}
  \begin{block}{Преимущества разработанного ПО}
    \fontsize{8.5}{10}\selectfont
    Self-hosted~$\cdot$~Версионирование без удаления~$\cdot$~Умные плейлисты~$\cdot$~Потоковая распаковка~$\cdot$~Открытый REST API
  \end{block}
\end{frame}
